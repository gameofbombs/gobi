...
// l = loaction
	...
	export function generateUniformsSync(group: UniformGroup, uniformData: any): any {
		let textureCount = 0;
		let func = `var v = null;
    var cv = null
    var gl = renderer.gl`;
		...
		for (const i in group.uniforms) {
            ...
            if(ud.${i}.value !== ${textureCount})
            {
				...
				const templateType = (data.size === 1) ? GLSL_TO_SINGLE_SETTERS_CACHED : GLSL_TO_ARRAY_SETTERS as any;
