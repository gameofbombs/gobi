namespace gobi.pixi {
	...
	import WebGLGraphics = gobi.pixi.graphicsUtils.WebGLGraphics;
	...
	import Ellipse = gobi.core.Ellipse;
	import IShape = gobi.core.IShape;
	...
	export interface IGraphics {
		...
		drawPolygon(path: Array<IPoint | number> | core.Polygon): void;
		clear(): void;
	}
	 ...
	 * @memberof PIXI
	 */
	export class GraphicsDisplayObject extends DisplayObject implements IGraphics {
		fillAlpha = 1;
		fillColor = 0;
		lineWidth = 0;
		lineAlpha = 1;
		...
		_webGL: { [key: number]: WebGLGraphics } = {};

		isMask = false;
		boundsPadding = 0;
		_localBounds = new Bounds();
		dirty: number = 0;
		fastRectDirty: number = 0;
		clearDirty: number = 0;
		boundsDirty: number = -1;
		cachedSpriteDirty: boolean = false;
		_spriteRect: Sprite = null;
		_fastRect: boolean = false;
		...
		lineStyle(lineWidth = 0, color = 0, alpha = 1) {
			...
			if (this.currentPath) {
				const poly = this.currentPath.shape as Polygon;
				if (poly.points.length) {
					// halfway through a line? start a new one!
					const shape = new Polygon(poly.points.slice(-2));
		...
		lineTo(x: number, y: number) {
			(this.currentPath.shape as Polygon).points.push(x, y);
			this.dirty++;
		...
		bezierCurveTo(cpX: number, cpY: number, cpX2: number, cpY2: number, toX: number, toY: number) {
			...
			graphicsUtils.bezierCurveTo(fromX, fromY, cpX, cpY, cpX2, cpY2, toX, toY, points);
		...
		arc(cx: number, cy: number, radius: number, startAngle: number, endAngle: number, anticlockwise = false) {
			...
			let points = this.currentPath ? (this.currentPath.shape as Polygon).points : null;
			...
			if (points) {
				...
				points = (this.currentPath.shape as Polygon).points;
			}
		...
		beginFill(color = 0, alpha = 1) {
			...
			if (this.currentPath) {
				if ((this.currentPath.shape as Polygon).points.length <= 2) {
		 ...
		 * @return {PIXI.Graphics} This Graphics object. Good for chaining method calls
		 */
		drawPolygon(path: Array<IPoint|number> | Polygon) {
			...
			if (points instanceof Polygon) {
				const poly = points as Polygon;
				closed = poly.closed;
				points = poly.points;
			}

			if (!Array.isArray(points)) {
				const arr = path as Array<IPoint|number>;
		 ...
		 * @param {PIXI.WebGLRenderer} renderer - The renderer
		 */
		_renderSpriteRect(renderer: WebGLRenderer) {
			const rect = this.graphicsData[0].shape as Rectangle;

			const node = this.node;

			if (!this._spriteRect) {
				this._spriteRect = new Sprite(new Texture(Texture.WHITE.baseTexture));
			}

			const sprite = this._spriteRect;

			//TODO: work with tint and alpha
			...
			sprite.gobiSprite.blendMode = this.blendMode;

			sprite.gobiSprite._texture._frame.width = rect.width;
			sprite.gobiSprite._texture._frame.height = rect.height;

			sprite.transform.worldTransform = node.transform.worldTransform;

			sprite.gobiSprite.anchor.set(-rect.x / rect.width, -rect.y / rect.height);
			sprite.gobiSprite._onAnchorUpdate();

			sprite.gobiSprite.renderWebGL(renderer);
		}
		...
		calculateBounds(bounds: Bounds) {
			...
			bounds.addFrame(node.transform, lb.minX, lb.minY, lb.maxX, lb.maxY);
		}
		 ...
		 */
		containsPoint(point: core.IPoint) {
			const node = this.node;
			node.worldTransform.applyInverse(point, tempPoint);
			...
			if (this.graphicsData.length) {
				...
				for (let i = 0; i < this.graphicsData.length; i++) {
					...
					if (type === SHAPES.RECT || type === SHAPES.RREC) {
						...
						x = r.x;
						y = r.y;
						w = r.radius + (lineWidth / 2);
						h = r.radius + (lineWidth / 2);
						...
						const r = shape as Ellipse;
						x = r.x;
						y = r.y;
						w = r.width + (lineWidth / 2);
						h = r.height + (lineWidth / 2);
		...
		drawShape(shape: IShape) {
			...
			if (data.type === SHAPES.POLY) {
				const poly = data.shape as Polygon;
				poly.closed = poly.closed || this.filling;
		...
		closePath() {
			...
			if (currentPath && currentPath.shape) {
				(currentPath.shape as Polygon).close();
			}
		...
		addHole() {
			...
			this.currentPath.addHole(hole.shape as Polygon);
			this.currentPath = null;
		...
		static _SPRITE_TEXTURE : Texture = null;
	}
}
