namespace gobi.pixi.systems {
	import GLFramebuffer = gobi.glCore.GLFramebuffer;
	export class FrameBufferSystem extends BaseSystem{
		gl: WebGLRenderingContext;
		CONTEXT_UID: number;
		drawBufferExtension: WebGLDrawBuffers;
		...
		bind(framebuffer: any) {
			...
			if (framebuffer) {
				...
				if (fbo.dirtyId !== framebuffer._updateID) {
					fbo.dirtyId = framebuffer._updateID;
		...
		initFramebuffer(framebuffer: any) {
			const gl = this.gl;
		...
		resizeFramebuffer(framebuffer: GLFramebuffer) {
			const gl = this.gl;
		...
		updateFramebuffer(framebuffer: any) {
			const gl = this.gl;
