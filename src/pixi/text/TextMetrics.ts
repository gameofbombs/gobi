namespace gobi.pixi {
	...
	const tempBounds = new gobi.core.Bounds();
	...
	export class TextMetrics {
		...
		constructor(text: string, style: TextStyle, width: number, height: number,
		            lines: Array<string>, lineWidths: Array<number>,
		            lineHeight: number, maxLineWidth: number, fontProperties: number) {
			this.text = text;
			this.style = style;
			this.width = width;
			this.height = height;
		...
		static measureText(text: string, style: TextStyle, wordWrap: boolean, canvas = TextMetrics._canvas) {
			...
			tempBounds.clear();
			tempBounds.addTuple(0, 0);
			if (style.dropShadow) {
				const d = style.dropShadowDistance;
				const a = style.dropShadowAngle;
				tempBounds.addTuple(d * Math.cos(a), d * Math.sin(a));
			}
			if (style.multiShadow) {
				const multi = style.multiShadow;
				for (let i = 0; i < multi.length;i++) {
					tempBounds.addTuple(multi[i].x, multi[i].y);
				}
			}
			tempBounds.ceil();
		...
		static wordWrap(text: string, style: TextStyle, canvas = TextMetrics._canvas) {
			...
			const characterCache: { [key: string]: number } = {};
