namespace gobi.utils.earcut {
	// This is TS port of https://github.com/mapbox/earcut

	export function earcut(data: Array<number>, holeIndices: Array<number>, dim: number) {
		...
		let minX, minY, maxX, maxY, x, y, size: number;
...
// create a circular doubly linked list from polygon points in the specified winding order
	function linkedList(data: Array<number>, start: number, end: number, dim: number, clockwise: boolean) {
...
// main ear slicing loop which triangulates a polygon (given as a linked list)
	function earcutLinked(ear: Node, triangles: Array<number>, dim: number, minX: number, minY: number, size: number, pass?: number) {
		if (!ear) return;
...
// try splitting polygon into two and triangulate them independently
	function splitEarcut(start: Node, triangles: Array<number>, dim: number, minX: number, minY: number, size: number) {
...
// link every hole into the outer loop, producing a single-ring polygon without holes
	function eliminateHoles(data: Array<number>, holeIndices: Array<number>, outerNode: Node, dim: number) {
		let queue : Array<Node> = [],
			i: number, len: number, start: number, end: number, list: Node;
...
// used to verify correctness of triangulation
	export function deviation(data: Array<number>, holeIndices: Array<number>, dim: number, triangles: Array<number>): number {
		let hasHoles = holeIndices && holeIndices.length;
	...
	function signedArea(data: Array<number>, start: number, end: number, dim: number) {
		let sum = 0;
...
// turn a polygon in a multi-dimensional array form (e.g. as in GeoJSON) into a form Earcut accepts
	export function flatten(data: Array<Array<Array<number>>>) {
		let dim = data[0][0].length,
			result: IFlatten = {vertices: [], holes: [], dimensions: dim},
			holeIndex = 0;
