...
// };
	 ...
	 */
	export function getResolutionOfUrl(url: string, defaultValue: number = 1) {
		const resolution = gobi.pixi.settings.RETINA_PREFIX.exec(url);
	...
	export function sayHello(type: string) {
		...
		if (navigator.userAgent.toLowerCase().indexOf('chrome') > -1) {
			const args = [
				`\n %c %c %c gobi.js ${VERSION} - ✰ ${type} pixi.js emulation mode ✰  %c  %c  http://www.pixijs.com/  %c %c ♥%c♥%c♥ \n\n`,
