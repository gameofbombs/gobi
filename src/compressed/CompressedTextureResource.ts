...
namespace gobi.IvanPerf {
	export class PerfPair {
		time = 0;
		src = "";

		constructor(time: number, src: string) {
			this.time = time;
		}

		addTo(arr: Array<PerfPair>, max: number) {
			arr.push(this);
	...
	export namespace CRN {
		export let totalCrnTime: number = 0;
		export let maxCrnTime: Array<PerfPair> = [];

		export function add(time: number, src: string) {
			totalCrnTime += time;
			new PerfPair(time, this.src).addTo(maxCrnTime, 5);
		}
	}
}


namespace gobi.compressed {
//===============//
// PVR constants //
//===============//
...
// Calcualates the size of a compressed texture level in bytes
	...
	export class CompressedImage implements core.ITextureResource, core.IDisposable {
		...
		baseTexture: core.BaseTexture = null;
		...
		onTextureUpload(renderer: pixi.WebGLRenderer, baseTexture: core.BaseTexture, glTexture: gobi.glCore.GLTexture): boolean {
		...
		_loadCRN(arrayBuffer: ArrayBuffer) {
			...
			IvanPerf.CRN.add(perfTime, this.src);
		...
		onTextureNew(baseTexture: core.BaseTexture) {
			if (!this.baseTexture) {
		...
		onTextureDestroy(baseTexture: core.BaseTexture) {
